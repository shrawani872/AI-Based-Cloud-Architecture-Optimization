...
Derives real-world cost metrics (hourly, daily, monthly) based on official AWS published
on-demand pricing for EC2, RDS, and attached resources. Computes idle capacity waste,
predictive cost trajectories, rightsizing recommendations, and dollar savings for
scaling actions.
"""
...
# Storage & Data Transfer Reference Costs
EBS_GP3_PER_GB_MONTH = 0.08
EGRESS_PER_GB = 0.09

# Default resource mappings for NAB and Bitbrains series
...
def resolve_instance_type(resource_id: str, requested_instance: Optional[str] = None) -> str:
    """Finds the configured or most representative instance type for a resource."""
...
def get_instance_specs(instance_type: str) -> Dict[str, Any]:
    """Retrieves instance specs and hourly rate from catalog."""
...
def _find_rightsized_instance(current_instance: str, peak_utilization: float) -> Tuple[Optional[str], float]:
    """
    Finds a rightsized smaller instance if utilization is safely low,
    or a larger instance if utilization is near saturation.
    Returns (suggested_instance, new_hourly_rate).
    """
    ...
    # Sort instances in the same family or general progression by hourly cost
    family_instances = [
        (name, spec["hourly"])
    ...
    if peak_utilization < 25.0 and current_idx > 0:
        # Candidate for scale in / step down
        # If very low (<10%), can step down 2 tiers if available
        ...
        # Candidate for scale out / step up
...
def estimate_cost_metrics(
    ...
    """
    Computes complete cost efficiency breakdown:
    - Base running cost (daily & monthly)
    - Effective utilized cost vs Idle waste cost
    - Efficiency score
    - Rightsizing opportunity & dollar savings
    """
    ...
    monthly_base_cost = daily_base_cost * 30.416  # average days per month

    # Determine dominant resource load
    ...
    # Peak load for rightsizing safety (taking higher of current or predicted)
...
def compute_scale_action_cost_impact(
    ...
    """
    Produces human-readable and machine-actionable cost and reliability impact strings
    to enrich recommendation objects.
    """
    ...
    if recommendation_type == "SCALE_IN":
        ...
        added_daily = cost_est.current_daily_cost_usd  # assuming +1 replica of current instance
        cost_impact = (
